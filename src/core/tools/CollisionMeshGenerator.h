...
public:
    ...
    LODResult generate(const QVector<QVector3D>& vertices,
                       const QVector<int>& indices,
                       const QVector<QVector3D>& normals = {},
                       const QVector<QVector2D>& uvs = {},
                       const LODOptions& options = LODOptions());

    LODResult generateFromFile(const QString& filePath,
                               const LODOptions& options = LODOptions());

    static LODLevel generateImpostor(const QVector<QVector3D>& vertices,
                                     const QVector<int>& indices,
                                     int resolution = 512);

    static float calculateScreenError(const LODLevel& lod,
                                      float distance,
                                      float fov,
                                      int viewportHeight);
};
...
public:
    ...
    MeshOperationResult apply(const QVector<QVector3D>& vertices,
                              const QVector<int>& indices,
                              const QVector<QVector3D>& normals = {},
                              const QVector<QVector2D>& uvs = {},
                              const MeshOperationOptions& options = MeshOperationOptions());

    QVector<MeshOperationResult> batchApply(const QVector<QPair<QVector<QVector3D>, QVector<int>>>& meshes,
                                             const MeshOperationOptions& options);
};
