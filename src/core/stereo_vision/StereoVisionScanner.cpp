...
void StereoVisionScanner::simulateStereoPair(CapturedPair& pair)
{
    ...
    for (int y = 0; y < m_imageHeight; y += 2) {
        leftScan = pair.leftImage.scanLine(y).cast<QRgb>();
    ...
    for (int y = 0; y < m_imageHeight; ++y) {
        rightScan = pair.rightImage.scanLine(y).cast<QRgb>();
        for (int x = 0; x < m_imageWidth; ++x) {
...
void StereoVisionScanner::processPair(const CapturedPair& pair)
{
    ...
    for (int y = 0; y < m_imageHeight; y += 8) {  // Sample every 8 pixels for demo
        ...
        for (int x = 0; x < m_imageWidth; x += 8) {
            ...
            uchar leftVal = qGray(leftScan[x]);
            uchar rightVal = qGray(rightScan[x]);
            ...
            // Search range
            int searchMin = m_disparityRange.min;
            int searchMax = qMin(m_disparityRange.max, m_imageWidth - x);
            
            for (int d = searchMin; d < searchMax; d += m_disparityRange.step) {
                // Compare windows (very simplified: just compare single pixel)
                if (d + x < m_imageWidth) {
                    uchar rightValAtShift = qGray(rightScan[qMin(m_imageWidth - 1, x + d)]);
    ...
    if (!m_result.points.isEmpty()) {
        ...
        for (const auto& pt : m_result.points) {
            minPt = minPt.qMin(pt.position);
            maxPt = maxPt.qMax(pt.position);
        }
...
ScanResult StereoVisionScanner::result() const
{
    return m_result;
}

QList<CapturedPair> StereoVisionScanner::pairs() const
{
    return m_pairs;
}
