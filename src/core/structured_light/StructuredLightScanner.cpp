...
ScanResult StructuredLightScanner::result() const
{
    return m_result;
}

QList<CapturedFrame> StructuredLightScanner::frames() const
{
    return m_frames;
}

void StructuredLightScanner::setBaseline(qreal b)
{
    m_baseline = b;
}

void StructuredLightScanner::setProjectorDistance(qreal d)
{
    m_projectorDistance = d;
}
...
void StructuredLightScanner::simulateCapture(PatternType type, QImage& outPattern, QImage& outCaptured)
{
    ...
    QRgb* scanLine = nullptr;
    for (int y = 0; y < outCaptured.height(); ++y) {
        scanLine = outCaptured.scanLine(y).cast<QRgb>();
        for (int x = 0; x < outCaptured.width(); ++x) {
            ...
            QRgb original = outPattern.pixel(nx, ny);
            scanLine[x] = original;
        }
    }
}
...
QImage StructuredLightScanner::generatePattern(PatternType type, const QSize& size)
{
    ...
    QRgb* scanLine = nullptr;
    for (int y = 0; y < h; ++y) {
        scanLine = pattern.scanLine(y).cast<QRgb>();
        for (int x = 0; x < w; ++x) {
            int val = 0;
