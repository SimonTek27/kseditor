...
void main() {
    ...
    if (pc.doubleSided > 0.5 && !gl_FrontFacing) {
        N = -N;
    }
    ...
    vec4 texDiffuse = texture(diffuseMap, fragTexCoord);
    vec3 baseColor = (pc.diffuseColor * texDiffuse).rgb;
    ...
    outColor = vec4(color, texDiffuse.a * pc.alpha);
}
