...
bool BankWriterFmod1x::writeProjectBanks(const KSAudioProject& project, const QString& outputDir,
    ...
    for (int i = 0; i < fspro.banks.size(); ++i) {
        ...
        // Write unencrypted first, then encrypt if needed
        QByteArray bankData;
        QDataStream memStream(&bankData, QIODevice::WriteOnly);
        memStream.setByteOrder(QDataStream::LittleEndian);
        memStream.setFloatingPointPrecision(QDataStream::SinglePrecision);

        writeFEV2Header(memStream, FEV2Header{});
        writeSTRTChunk(memStream, strTableData);
        writeEVTSChunk(memStream, eventEntries);

        {
            ...
            memStream << static_cast<quint32>(0x42555353);
            memStream << static_cast<quint32>(busData.size());
            memStream.device()->write(busData);
        }

        writeVCASChunk(memStream, fspro.vcas, strMap);
        writeSNDSChunk(memStream, soundEntries);

        for (const auto& pcm : audioBuffers) {
            if (!pcm.isEmpty())
                memStream.device()->write(pcm);
        }

        // Apply encryption if enabled
        QByteArray finalData = bankData;
        if (m_encrypt) {
            finalData = encryptData(bankData);
        }
...
bool BankWriterFmod1x::writeBank(const SoundBank& bank, const QString& assetsDir,
...
bool BankWriterFmod1x::writeGUIDsFile(const KSAudioProject& project, const QString& outputPath) {
    QFile f(outputPath);
...
bool BankWriterFmod1x::convertKSAudioToBank(const QString& ksaudioPath,
                                            const QString& assetsDir,
...
void BankWriterFmod1x::writeFEV2Header(QDataStream& s, const FEV2Header& hdr) {
    s << hdr.magic << hdr.version << hdr.flags;
}

void BankWriterFmod1x::writeSTRTChunk(QDataStream& s, const QByteArray& strTable) {
    s << static_cast<quint32>(0x53545254) << static_cast<quint32>(strTable.size());
    s.device()->write(strTable);
}

void BankWriterFmod1x::writeEVTSChunk(QDataStream& s, const QVector<EventEntry>& events) {
    QByteArray data;
...
void BankWriterFmod1x::writeBUSSEntry(QDataStream& s, const FSPROBus& bus,
...
void BankWriterFmod1x::writeVCASChunk(QDataStream& s, const QVector<FSPROVCA>& vcas,
                                      const QMap<QString, quint32>& strMap) {
    QByteArray data;
    QDataStream ds(&data, QIODevice::WriteOnly);
    ds.setByteOrder(QDataStream::LittleEndian);
...
void BankWriterFmod1x::writeSNDSChunk(QDataStream& s, const QVector<SoundEntry>& sounds) {
    QByteArray data;
    QDataStream ds(&data, QIODevice::WriteOnly);
    ds.setByteOrder(QDataStream::LittleEndian);
...
QByteArray BankWriterFmod1x::makeGUID(const QString& name) {
...
bool BankWriterFmod1x::readAudioData(const QString& filePath, QByteArray& outData,
...
QByteArray BankWriterFmod1x::encryptData(const QByteArray& data) const {
    QByteArray encrypted = data;
    quint32* words = reinterpret_cast<quint32*>(encrypted.data());
    int wordCount = encrypted.size() / 4;
    quint32 prev = 0;
    for (int i = 0; i < wordCount; ++i) {
        quint32 cur = words[i];
        words[i] ^= m_xorKey ^ prev;
        prev = cur;
    }
    // Update flags in header to indicate encryption
    if (encrypted.size() >= 12) {
        quint32* flags = reinterpret_cast<quint32*>(encrypted.data() + 8);
        *flags |= 0x1;
    }
    return encrypted;
}

QMap<QString, quint32> BankWriterFmod1x::buildStringTable(const FSPROProject& proj, QByteArray& strTableData) {
    QMap<QString, quint32> strMap;
    auto regStr = [&](const QString& s) -> quint32 {
        if (strMap.contains(s)) return strMap[s];
        quint32 idx = static_cast<quint32>(strMap.size());
    ...
    for (const auto& eg : proj.eventGroups) {
        for (const auto& e : eg.events) {
            regStr(e.name);
            regStr("event:/" + e.name);
        }
    }
    for (const auto& b : proj.buses) {
        regStr(b.name);
        regStr("bus:/" + b.name);
    }
    for (const auto& v : proj.vcas) {
        regStr(v.name);
        regStr("vca:/" + v.name);
    }
    for (const auto& s : proj.sounds) {
        regStr(s.name);
    }
