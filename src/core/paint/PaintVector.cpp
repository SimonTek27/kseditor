...
QString PaintVectorDocument::filterSvg(const QString& id) const{ if(!m_filterObjs.contains(id)) return ""; QString s=QString("<filter id='%1'>").arg(id); for(auto &p:m_filterObjs[id].primitives){ s+=QString("<feGaussianBlur in='%1' stdDeviation='%2'/>").arg(p.in).arg(p.params.value("stdDeviation","2").toString()); } s+="</filter>"; return s; }
...
QVariantMap PaintVectorDocument::extensionInfo(const QString& id) const{ if(!m_extensions.contains(id)) return {}; QVariantMap m; const auto &e=m_extensions.value(id); m["id"]=e.id; m["name"]=e.name; m["script"]=e.scriptPath; m["menu"]=e.menuLabel; m["enabled"]=e.enabled; return m; }
