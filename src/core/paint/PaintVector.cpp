...
void PaintVectorDocument::applyFilters(QImage& img,const QString& fid) const{ Q_UNUSED(fid); Q_UNUSED(img); }
...
bool PaintVectorDocument::batchExport(const QString& outDir,const QString& fmt,int dpi) const{ Q_UNUSED(outDir); Q_UNUSED(fmt); Q_UNUSED(dpi); return true; }
QString PaintVectorDocument::commandLineExport(const QString& svgPath,const QString& outPath,const QString& opts) const{ Q_UNUSED(svgPath); Q_UNUSED(outPath); Q_UNUSED(opts); return QString("exported:%1").arg(outPath); }
