...
KN5File KN5ParserImpl::parse(const QString& filePath, QString* error) {
    ...
    try {
        ...
        if (h.version < KN5_VERSION_MIN || h.version > KN5_VERSION) {
            m_lastError = QString("Unsupported KN5 version: %1 (supported %2-%3)").arg(h.version).arg(KN5_VERSION_MIN).arg(KN5_VERSION);
            if (error) *error = m_lastError;
            return KN5File();
        }

        // Read textures (parseTexture appends, do NOT pre-allocate)
...
void KN5ParserImpl::parseMaterial(KN5File& out, QDataStream& stream) {
    ...
    quint32 matType;
    stream >> matType;
    mat.type = static_cast<Material::Type>(matType);
...
void KN5ParserImpl::parseMesh(KN5File& out, QDataStream& stream) {
    ...
    quint32 meshMatType;
    stream >> meshMatType;
    mesh.materialType = static_cast<Mesh::MaterialType>(meshMatType);
    ...
    // Detect skinned meshes from vertex layout
    mesh.isSkinnedMesh = mesh.vertexLayout.has(AttributeType::BoneWeight);
