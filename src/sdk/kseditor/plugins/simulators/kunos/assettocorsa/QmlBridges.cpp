...
void LiveryEditorBridge::loadSkins(const QString& carPath)
{
    ...
    m_manager = new graphics::PaintManager(carPath);
    m_manager->loadSkins();
...
{
    bool ok = graphics::PaintSystem::createSkin(carPath, skinName);
    if (ok) {
        loadSkins(carPath);
...
{
    bool ok = graphics::PaintSystem::deleteSkin(carPath, skinName);
    if (ok) {
        loadSkins(carPath);
...
{
    bool ok = graphics::PaintSystem::duplicateSkin(carPath, sourceName, destName);
    if (ok) {
        loadSkins(carPath);
    }
    return ok;
...
{
    graphics::PaintSystem::SkinConfig config = graphics::PaintSystem::loadSkinConfig(skinPath);
...
{
    graphics::PaintSystem::SkinConfig sc;
    ...
    for (const QVariant& v : layersList) {
        ...
        graphics::PaintSystem::PaintLayer layer;
    ...
    bool ok = graphics::PaintSystem::saveSkinConfig(sc, skinPath);
                                   ...
                                   float opacity, float posX, float posY, float sizeW, float sizeH)
{
    ...
    bool ok = graphics::PaintSystem::addLayer(config, layer);
    if (ok) {
        ok = graphics::PaintSystem::saveSkinConfig(config, skinPath);
        if (ok) emit liveryModified();
    }
    return ok;
}
...
{
    graphics::PaintSystem::SkinConfig config = graphics::PaintSystem::loadSkinConfig(skinPath);
    bool ok = graphics::PaintSystem::removeLayer(config, index);
    if (ok) {
        ok = graphics::PaintSystem::saveSkinConfig(config, skinPath);
        if (ok) emit liveryModified();
    }
...
{
    graphics::PaintSystem::SkinConfig config = graphics::PaintSystem::loadSkinConfig(skinPath);
    bool ok = graphics::PaintSystem::moveLayer(config, fromIndex, toIndex);
    if (ok) {
        ok = graphics::PaintSystem::saveSkinConfig(config, skinPath);
        if (ok) emit liveryModified();
    }
...
bool LiveryEditorBridge::generateLicensePlate(const QString& skinPath, const QString& text, const QString& country)
{
    ...
    bool ok = graphics::PaintSystem::generateLicensePlate(text, country, outputPath);
    if (ok) {
        graphics::PaintSystem::SkinConfig config = graphics::PaintSystem::loadSkinConfig(skinPath);
        graphics::PaintSystem::PaintLayer layer;
...
{
    return graphics::PaintSystem::generatePreview(skinPath);
}

bool LiveryEditorBridge::validateSkin(const QString& skinPath)
{
    QString error;
    bool valid = graphics::PaintSystem::validateSkin(skinPath, &error);
...
{
    return graphics::PaintSystem::exportSkin(skinPath, outputPath);
}

bool LiveryEditorBridge::importSkin(const QString& importPath, const QString& carPath)
{
    bool ok = graphics::PaintSystem::importSkin(importPath, carPath);
