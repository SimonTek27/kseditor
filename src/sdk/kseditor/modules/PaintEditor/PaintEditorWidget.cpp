...
void PaintEditorWidget::setupUI()
{
    ...
    m_paintEditor = new ks::graphics::PaintEditor(m_canvasStack);
    ...
    m_viewport3D = new ks::graphics::PaintViewport(canvasArea);
    ...
    connect(m_paintEditor, &ks::graphics::PaintEditor::textureChanged, this, [this](const QImage& tex) {
        if (m_viewport3D)
            m_viewport3D->applyPaintTexture(tex);
    });

    connect(m_paintEditor, &ks::graphics::PaintEditor::imageEdited, this, [this]() {
        emit paintModified();
    });

    // Update undo/redo button states from paint editor history
    connect(m_paintEditor, &ks::graphics::PaintEditor::historyChanged, this, [this]() {
