...
class ModellingRenderEngine : public QObject
{
    Q_OBJECT
public:
    explicit ModellingRenderEngine(QObject* parent = nullptr) : QObject(parent) {}
    ~ModellingRenderEngine() {}

    enum RenderEngineType { Eevee, Cycles, OpenGL, Vulkan };

    void setEngine(RenderEngineType type) { m_engine = type; }
    RenderEngineType engine() const { return m_engine; }

    void setScene(Scene3D* scene) { m_scene = scene; }
    Scene3D* scene() const { return m_scene; }
