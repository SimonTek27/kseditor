...
TrackBuilderModule::TrackBuilderModule(QObject* parent) : QObject(parent)
{
    ...
    m_vegetation  = new VegetationScatter(this);
...
// ============================================================================
// Vegetation scatter tools
// ============================================================================
int TrackBuilderModule::addVegetationLayer(const QString& name, int type)
{
    if (!m_vegetation) return -1;

    // Sync terrain data to vegetation system
    m_vegetation->setTerrainData(m_terrain->heightmap(),
                                  m_terrain->gridW(), m_terrain->gridH(),
                                  m_terrain->worldW(), m_terrain->worldH());
    m_vegetation->setTerrainNormals(m_terrain->normals());

    auto preset = VegetationScatter::defaultPreset(static_cast<VegetationType>(type));
    return m_vegetation->addLayer(name, preset);
}

void TrackBuilderModule::removeVegetationLayer(int index)
{
    if (m_vegetation) m_vegetation->removeLayer(index);
}

void TrackBuilderModule::paintVegetationDensity(int layerIndex, float wx, float wz,
                                                float radius, float opacity)
{
    if (!m_vegetation) return;

    // Ensure terrain data is synced
    m_vegetation->setTerrainData(m_terrain->heightmap(),
                                  m_terrain->gridW(), m_terrain->gridH(),
                                  m_terrain->worldW(), m_terrain->worldH());

    m_vegetation->paintDensity(layerIndex, wx, wz, radius, opacity);
    markDirty();
}

void TrackBuilderModule::autoMaskVegetationBySlope(int layerIndex, float minSlope, float maxSlope)
{
    if (!m_vegetation) return;
    ...
    m_vegetation->autoMaskBySlope(layerIndex, minSlope, maxSlope);
    markDirty();
}

void TrackBuilderModule::generateVegetation()
{
    if (!m_vegetation) return;

    // Sync terrain data before generation
    m_vegetation->setTerrainData(m_terrain->heightmap(),
                                  m_terrain->gridW(), m_terrain->gridH(),
                                  m_terrain->worldW(), m_terrain->worldH());
    m_vegetation->setTerrainNormals(m_terrain->normals());

    m_vegetation->generateAllInstances();
    markDirty();
}

void TrackBuilderModule::generateVegetationLayer(int layerIndex)
{
    if (!m_vegetation) return;
    ...
    m_vegetation->generateInstances(layerIndex);
    markDirty();
}

int TrackBuilderModule::vegetationInstanceCount() const
{
    if (!m_vegetation) return 0;
    return m_vegetation->allInstances().size();
}
