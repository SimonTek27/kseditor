...
public:
    ...
    // ---- Vegetation scatter ------------------------------------------------
    Q_INVOKABLE int     addVegetationLayer(const QString& name, int type);
    Q_INVOKABLE void    removeVegetationLayer(int index);
    Q_INVOKABLE void    paintVegetationDensity(int layerIndex, float wx, float wz,
                                               float radius, float opacity);
    Q_INVOKABLE void    autoMaskVegetationBySlope(int layerIndex, float minSlope, float maxSlope);
    Q_INVOKABLE void    generateVegetation();
    Q_INVOKABLE void    generateVegetationLayer(int layerIndex);
    Q_INVOKABLE int     vegetationLayerCount() const { return m_vegetation ? m_vegetation->layerCount() : 0; }
    Q_INVOKABLE int     vegetationInstanceCount() const;

    // ---- Sub-system access ------------------------------------------------
    TerrainEngine* terrain()   { return m_terrain; }
    RoadBuilder*   roadBuilder(){ return m_roadBuilder; }
    TrackProject&  project()   { return m_project; }
    VegetationScatter* vegetationScatter() { return m_vegetation; }
...
private:
    ...
    VegetationScatter* m_vegetation  = nullptr;
    bool              m_dirty        = false;
