...
// ============================================================================
// AC CSP digital_instruments.ini / analog_instruments.ini structures
// ============================================================================

enum class AcItemType {
    GEAR, SPEED, RPM, LAPTIME, FUEL, PERF, PRESSURE,
    TC_LEVEL, WATER_TEMP, LAST_LAP, FUEL_CONS,
    KERS_CHARGE, CURRENT_LAP, POSITION_CAR, GEAR_COLOR,
    BEST_LAP, PLACE_HOLDER, UNKNOWN
};

enum class AcDisplayParent {
    DISPLAY_WHEEL,   // steering wheel screen
    DISPLAY_MONITOR, // dash LCD
    UNKNOWN
};

struct AcDisplayItem {
    int index = 0;
    AcItemType type = AcItemType::UNKNOWN;
    AcDisplayParent parent = AcDisplayParent::DISPLAY_WHEEL;
    QString objectName;
    QPoint position;
    QSize size;
    QColor color;
    ...
    int intensity = 1;
    int intensity2 = 1;
    QString font = "german_led";
    int version = 2;
    QString align = "CENTER";   // LEFT, CENTER, RIGHT
    int decimals = 0;
    QString prefix;
    QString postfix;
    QString units;              // KPH, KML
    int tyreNumber = -1;        // 0..3 or -1 for none
    double nTime = 0.1;
    int rpmTrigger = 0;         // for GEAR_COLOR shift light
    bool visible = true;
};
...
struct AcTyreLockSlip {
    int index = 0;
    QString objectName;
    int tyreIndex = 0;          // 0=left, 1=right
    QColor emissive;
    QColor emissiveLock;
    double diffuse = 0.25;
    double slipSwitch = 0.6;
    bool showLock = false;
    double wheelSpeedMult = 0.8;
    bool visible = true;
};

struct AcKersChargeSerie {
    int index = 0;
    QString prefix = "KERS_LEVEL_";
    int startIndex = 1;
    int endIndex = 34;
};

// ============================================================================
// Legacy display element structures (for custom INI/Lua/JSON formats)
// ============================================================================
...
class ksCockpitInstruments : public QObject
{
    Q_OBJECT

public:
    explicit ksCockpitInstruments(QObject *parent = nullptr);
    ~ksCockpitInstruments();
    ...
    // Element management (legacy format)
    ...
    // AC CSP format management
    QVector<AcDisplayItem>& getAcItems();
    QVector<AcLed>& getAcLeds();
    QVector<AcTyreLockSlip>& getAcTyreSlips();
    QVector<AcKersChargeSerie>& getAcKersSeries();
    void addAcItem(const AcDisplayItem& item);
    void addAcLed(const AcLed& led);
    void addAcTyreSlip(const AcTyreLockSlip& slip);
    void removeAcItem(int index);
    void removeAcLed(int index);
    void removeAcTyreSlip(int index);
    AcDisplayItem* getAcItem(int index);
    AcLed* getAcLed(int index);
    void clearAcItems();
    void clearAcLeds();
    void clearAcTyreSlips();
...
private:
    ...
    // AC CSP format data
    QVector<AcDisplayItem> m_acItems;
    QVector<AcLed> m_acLeds;
    QVector<AcTyreLockSlip> m_acTyreSlips;
    QVector<AcKersChargeSerie> m_acKersSeries;
    ...
    void parseAcItem(const QString& section, const QMap<QString, QString>& values);
    void parseAcLed(const QString& section, const QMap<QString, QString>& values);
    void parseAcTyreSlip(const QString& section, const QMap<QString, QString>& values);
    QString generateLuaScript() const;
    QString generateIniContent() const;
    QString generateAcIniContent() const;
    ...
    QColor rgbStringToColor(const QString& rgb) const;
