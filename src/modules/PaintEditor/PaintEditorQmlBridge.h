...
class PaintEditorQmlBridge : public QObject {
    Q_OBJECT
...
public:
    static PaintEditorQmlBridge* instance();
...
private:
    static PaintEditorQmlBridge* s_instance;
    PaintEditorQmlBridge(QObject* parent = nullptr) : QObject(parent) {}

    QVariantMap layerToVariant(const PaintSystem::PaintLayer& layer) const;
    PaintSystem::PaintLayer variantToLayer(const QVariantMap& v) const;

    QString m_carPath;
    QString m_currentSkin;
    QVector<PaintSystem::SkinInfo> m_skins;
    std::unique_ptr<PaintManager> m_manager;
};
