...
// ============================================================================
// PaintSystem — Skin management
// ============================================================================

QVector<PaintSystem::SkinInfo> PaintSystem::getSkins(const QString& carPath)
{
...
PaintSystem::SkinInfo PaintSystem::getSkinInfo(const QString& skinPath)
{
    SkinInfo info;
    info.path = skinPath;
...
bool PaintSystem::createSkin(const QString& carPath, const QString& skinName)
{
...
bool PaintSystem::deleteSkin(const QString& carPath, const QString& skinName)
{
...
bool PaintSystem::duplicateSkin(const QString& carPath, const QString& sourceName, const QString& destName)
{
...
// ============================================================================
// PaintSystem — Skin configuration (INI)
// ============================================================================

PaintSystem::SkinConfig PaintSystem::loadSkinConfig(const QString& skinPath)
{
    SkinConfig config;
...
bool PaintSystem::saveSkinConfig(const SkinConfig& config, const QString& skinPath)
{
    ...
    for (const PaintLayer& layer : config.layers) {
        QJsonObject obj;
        obj["name"] = layer.name;
        obj["type"] = layer.type;
...
bool PaintSystem::loadFromIni(SkinConfig& config, const QString& iniPath)
{
    QFile file(iniPath);
...
bool PaintSystem::saveToIni(const SkinConfig& config, const QString& iniPath)
{
    QFile file(iniPath);
...
// ============================================================================
// PaintSystem — Layer operations
// ============================================================================

bool PaintSystem::addLayer(SkinConfig& config, const PaintLayer& layer)
{
...
bool PaintSystem::removeLayer(SkinConfig& config, int index)
{
...
bool PaintSystem::moveLayer(SkinConfig& config, int fromIndex, int toIndex)
{
...
bool PaintSystem::updateLayer(SkinConfig& config, int index, const PaintLayer& layer)
{
    if (index < 0 || index >= config.layers.size()) return false;
    if (!validateLayer(layer)) return false;
...
// ============================================================================
// PaintSystem — License plate
// ============================================================================

bool PaintSystem::generateLicensePlate(const QString& text, const QString& country,
...
QStringList PaintSystem::getSupportedCountries()
{
    ks::LicensePlatesManager manager;
    return manager.availableCountries();
}

bool PaintSystem::isValidPlateText(const QString& text, const QString& country)
{
    ks::LicensePlatesManager manager;
...
// ============================================================================
// PaintSystem — Preview generation
// ============================================================================

bool PaintSystem::generatePreview(const QString& skinPath)
{
...
QImage PaintSystem::renderVectorPreview(const QString& vectorData, int width, int height)
{
    return renderVectorDataToImage(vectorData, width, height);
}

bool PaintSystem::hasPreview(const QString& skinPath)
{
    return QFile::exists(getPreviewPath(skinPath));
}

QString PaintSystem::getPreviewPath(const QString& skinPath)
{
    return skinPath + "/preview.jpg";
}

// ============================================================================
// PaintSystem — Export / Import
// ============================================================================

bool PaintSystem::exportSkin(const QString& skinPath, const QString& outputPath)
{
...
bool PaintSystem::importSkin(const QString& importPath, const QString& carPath)
{
...
// ============================================================================
// PaintSystem — Validation
// ============================================================================

bool PaintSystem::validateSkin(const QString& skinPath, QString* error)
{
    QDir skinDir(skinPath);
    if (!skinDir.exists()) {
...
bool PaintSystem::validateLayer(const PaintLayer& layer, QString* error)
{
...
QStringList PaintSystem::getLayerTypes()
{
    return QStringList() << "decal" << "paint" << "texture" << "vector";
}

QString PaintSystem::getDefaultSkinName()
{
    return "default";
}

bool PaintSystem::isDefaultSkin(const QString& skinName)
{
    return skinName.toLower() == "default";
}

// ============================================================================
// PaintSystem — Private helpers
// ============================================================================

bool PaintSystem::createDefaultFiles(const QString& skinPath)
{
    QFile iniFile(skinPath + "/skin.ini");
...
bool PaintSystem::createPreviewTexture(const QString& skinPath)
{
    return generatePreview(skinPath);
}

// ============================================================================
// PaintManager — High-level interface
// ============================================================================

PaintManager::PaintManager(const QString& carPath)
    : m_carPath(carPath)
{
}

bool PaintManager::loadSkins()
{
    m_skins = PaintSystem::getSkins(m_carPath);
    return true;
}

bool PaintManager::createSkin(const QString& name)
{
    bool ok = PaintSystem::createSkin(m_carPath, name);
    if (ok) {
        loadSkins();
    }
    return ok;
...
bool PaintManager::deleteSkin(const QString& name)
{
    bool ok = PaintSystem::deleteSkin(m_carPath, name);
    if (ok) {
        loadSkins();
    }
    return ok;
...
bool PaintManager::duplicateSkin(const QString& sourceName, const QString& destName)
{
    bool ok = PaintSystem::duplicateSkin(m_carPath, sourceName, destName);
    if (ok) {
        loadSkins();
    }
    return ok;
...
bool PaintManager::setCurrentSkin(const QString& skinName)
{
    for (const PaintSystem::SkinInfo& skin : m_skins) {
        if (skin.name == skinName) {
            m_currentSkin = skinName;
            m_config = PaintSystem::loadSkinConfig(skin.path);
            return true;
        }
    }
    return false;
...
bool PaintManager::saveCurrentSkin()
{
    if (m_currentSkin.isEmpty()) return false;

    QString skinPath = m_carPath + "/skins/" + m_currentSkin;
    return PaintSystem::saveSkinConfig(m_config, skinPath);
}

bool PaintManager::addLayer(const PaintSystem::PaintLayer& layer)
{
    return PaintSystem::addLayer(m_config, layer);
}

bool PaintManager::removeLayer(int index)
{
    return PaintSystem::removeLayer(m_config, index);
}

bool PaintManager::generateLicensePlate(const QString& text, const QString& country)
{
    if (m_currentSkin.isEmpty()) return false;
...
QVector<PaintSystem::UndoAction> PaintSystem::s_undoStack;
QVector<PaintSystem::UndoAction> PaintSystem::s_redoStack;

bool PaintSystem::exportSkinAsDDS(const QString& skinPath, const QString& outputPath)
{
    QImage paint(skinPath + "/paint.png");
    if (paint.isNull()) {
        paint = QImage(skinPath + "/sides_1.png");
    }
    if (paint.isNull()) return false;

    return saveTextureAsDDS(paint, outputPath);
}

bool PaintSystem::saveTextureAsDDS(const QImage& image, const QString& outputPath)
{
...
bool PaintSystem::importDecal(const QString& decalPath, const QString& skinPath)
{
...
QStringList PaintSystem::getSupportedDecalFormats()
{
...
QVector<PaintSystem::PaintTemplate> PaintSystem::getBuiltinTemplates()
{
    QVector<PaintTemplate> templates;
...
bool PaintSystem::createSkinFromTemplate(const QString& carPath, const QString& skinName,
                                            const PaintTemplate& tmpl)
{
    QString skinPath = carPath + "/skins/" + skinName;
    QDir().mkpath(skinPath);

    // Create base paint texture
    ...
    // Save base texture
    base.save(skinPath + "/paint.png", "PNG");
...
void PaintSystem::pushUndo(const UndoAction& action)
{
...
bool PaintSystem::canUndo()
{
    return !s_undoStack.isEmpty();
}

bool PaintSystem::canRedo()
{
    return !s_redoStack.isEmpty();
}

PaintSystem::UndoAction PaintSystem::undoLast()
{
...
PaintSystem::UndoAction PaintSystem::redoLast()
{
...
void PaintSystem::clearUndoRedo()
{
    s_undoStack.clear();
    s_redoStack.clear();
}
...
QVector<PaintSystem::ColorSwatch> PaintSystem::getDefaultPalette()
{
    return {
...
QVector<PaintSystem::ColorSwatch> PaintSystem::loadPalette(const QString& path)
{
...
bool PaintSystem::savePalette(const QVector<ColorSwatch>& palette, const QString& path)
{
    QFile file(path);
