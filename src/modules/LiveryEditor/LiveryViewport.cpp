    ...
    , m_liveryEditor(LiveryEditor::instance())
{
    ...
    connect(resetBtn, &QPushButton::clicked, this, &LiveryViewport::resetCamera);
...
void LiveryViewport::loadMesh(const QString& filePath)
{
    if (!m_meshRenderer) {
...
void LiveryViewport::convertToScene()
{
    delete m_sceneRoot;
    ...
    // Apply livery texture if available
    if (!m_liveryTexture.isNull()) {
        applyLiveryTexture(m_liveryTexture);
    }
}

void LiveryViewport::applyLiveryTexture(const QImage& texture)
{
    m_liveryTexture = texture;
    if (!m_sceneRoot) return;

    QImage sample = texture;
    if (sample.isNull()) return;

    // Sample the livery texture at each vertex's UV coordinates
