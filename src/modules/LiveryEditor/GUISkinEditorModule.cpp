...
void GUISkinEditorModule::shutdown() { if (m_statusLabel) m_statusLabel->setText("Shut down"); }

QDockWidget* GUISkinEditorModule::getOrCreateDockWidget(QMainWindow* mainWindow)
{
    if (m_dockWidget) return m_dockWidget;
    m_dockWidget = new QDockWidget("GUI Skin Editor", mainWindow);
    ...
    auto makeColorRow = [&](int row, const QString& label, QPushButton*& btn, auto slot) {
        btn = new QPushButton("Pick");
        colorsLayout->addWidget(new QLabel(label), row, 0);
        colorsLayout->addWidget(btn, row, 1);
        connect(btn, &QPushButton::clicked, this, slot);
    };

    makeColorRow(0, "Window BG:", m_windowBgBtn, &GUISkinEditorModule::onWindowBgClicked);
    makeColorRow(1, "Panel BG:", m_panelBgBtn, &GUISkinEditorModule::onPanelBgClicked);
    makeColorRow(2, "Text:", m_textColorBtn, &GUISkinEditorModule::onTextColorClicked);
    makeColorRow(3, "Accent:", m_accentColorBtn, &GUISkinEditorModule::onAccentColorClicked);
    makeColorRow(4, "Highlight:", m_highlightColorBtn, &GUISkinEditorModule::onHighlightColorClicked);
    makeColorRow(5, "Border:", m_borderColorBtn, &GUISkinEditorModule::onBorderColorClicked);
    makeColorRow(6, "Error:", m_errorColorBtn, &GUISkinEditorModule::onErrorColorClicked);
    makeColorRow(7, "Success:", m_successColorBtn, &GUISkinEditorModule::onSuccessColorClicked);
    makeColorRow(8, "Warning:", m_warningColorBtn, &GUISkinEditorModule::onWarningColorClicked);

    mainLayout->addWidget(colorsGroup);

    auto* actionLayout = new QHBoxLayout();
    m_loadBtn = new QPushButton("Load skin.ini"); m_saveBtn = new QPushButton("Save skin.ini"); m_resetBtn = new QPushButton("Reset");
...
{
    QString p = QFileDialog::getOpenFileName(this, "Open skin.ini", QString(), "Skin INI (*.ini)");
    if (!p.isEmpty()) { m_filePath = p; loadFileToUI(); m_statusLabel->setText("Loaded: " + p); }
}

void GUISkinEditorModule::onSaveFile()
{
    QString p = m_filePath.isEmpty() ? QFileDialog::getSaveFileName(this, "Save skin.ini", QString(), "Skin INI (*.ini)") : m_filePath;
...
void GUISkinEditorModule::setupUi() { if (m_statusLabel) m_statusLabel->setText("UI Ready"); }
