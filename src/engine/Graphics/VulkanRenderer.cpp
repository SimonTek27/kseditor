...
void VulkanRenderPass::setPushConstantRange(VkShaderStageFlags stageFlags, uint32_t size) {
    m_hasPushConstants = true;
    m_pushConstantStageFlags = stageFlags;
    m_pushConstantSize = size;
}

void VulkanRenderPass::pushConstants(VkCommandBuffer cmdBuffer, const void* data, uint32_t size) {
    if (cmdBuffer && m_pipelineLayout && m_hasPushConstants && data && size > 0) {
        g_vk.cmdPushConstants(cmdBuffer, m_pipelineLayout, m_pushConstantStageFlags, 0, size, data);
    }
}
...
bool VulkanRenderPass::compile() {
    ...
    if (m_hasPushConstants && m_pushConstantSize > 0) {
        pushRange.stageFlags = m_pushConstantStageFlags;
        pushRange.offset = 0;
        pushRange.size = m_pushConstantSize;
