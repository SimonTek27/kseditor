...
bool KsVulkanIntegration::initialize(const QString& systemPath)
{
    ...
    // Copy config settings into engine-local storage
    const auto& cfgGfx = KsConfigLoader::instance().graphicsSettings();
    m_graphicsSettings.maxFrameLatency = cfgGfx.maxFrameLatency;
    m_graphicsSettings.mipLodBias = cfgGfx.mipLodBias;
    m_graphicsSettings.shadowMapBias0 = cfgGfx.shadowMapBias0;
    m_graphicsSettings.shadowMapBias1 = cfgGfx.shadowMapBias1;
    m_graphicsSettings.shadowMapBias2 = cfgGfx.shadowMapBias2;
    m_graphicsSettings.skyboxReflectionGain = cfgGfx.skyboxReflectionGain;
    m_graphicsSettings.allowUnsupportedDX10 = cfgGfx.allowUnsupportedDX10;

    const auto& cfgLight = KsConfigLoader::instance().lightingSettings();
    m_lightingSettings.ambientColor[0] = cfgLight.ambientColor.redF();
    m_lightingSettings.ambientColor[1] = cfgLight.ambientColor.greenF();
    m_lightingSettings.ambientColor[2] = cfgLight.ambientColor.blueF();
    m_lightingSettings.ambientIntensity = cfgLight.lightIntensity * 100.0f;
    m_lightingSettings.hdrExposure = cfgLight.lightIntensity;
    ...
    m_ssgiRenderer = new ks::SSGIRenderer(m_renderer);
    if (m_ssgiRenderer->initialize({m_renderer->swapchainExtent().width, m_renderer->swapchainExtent().height})) {
        m_ssgiEnabled = true;
        LOG_INFO("KsVulkanIntegration", "SSGI initialized successfully");
    } else {
        qWarning() << "SSGI initialization failed, disabling";
        m_ssgiEnabled = false;
        delete m_ssgiRenderer;
        m_ssgiRenderer = nullptr;
    }
...
{
    const GraphicsSettings& settings = m_graphicsSettings;
...
{
    const LightingSettings& settings = m_lightingSettings;

    if (m_ppFilterRenderer) {
        ks::VulkanShaderLoader::LightingSettings light;
        light.ambientColor[0] = settings.ambientColor[0];
        light.ambientColor[1] = settings.ambientColor[1];
        light.ambientColor[2] = settings.ambientColor[2];
        light.ambientIntensity = settings.ambientIntensity;
        light.hdrExposure = settings.hdrExposure;
