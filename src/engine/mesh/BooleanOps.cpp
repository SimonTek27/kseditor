...
std::string BooleanOperations::validateMesh(const GeoMeshData& mesh) {
    if (mesh.vertices.empty()) return "Mesh has no vertices";
    if (mesh.faces.empty()) return "Mesh has no faces";
    return "";
}

BoolOpResult BooleanOperations::performOperation(
    const GeoMeshData& meshA, const GeoMeshData& meshB, Operation op) {
...
BoolOpResult BooleanOperations::performOperationImpl(
    ...
    try {
        ...
        for (auto vertex : cgalResult.vertices()) {
            ...
            result.result.vertices.push_back(GeoVertex(
        ...
        for (auto face : cgalResult.faces()) {
            ...
            result.result.faces.push_back(GeoFace(
                static_cast<uint32_t>(idxs[0]),
                static_cast<uint32_t>(idxs[1]),
                static_cast<uint32_t>(idxs[2])));
        }
        ...
        result.executionTimeMs = timer.elapsed();
        qWarning() << "Boolean operation failed:" << e.what();
    } catch (...) {
        result.status = BoolOpResult::ComputationFailed;
...
#else
    // Fallback: AABB overlap test for disjoint detection
    struct Vec3 { float x = 0, y = 0, z = 0; };
    auto computeAABB = [](const GeoMeshData& mesh) -> std::pair<Vec3, Vec3> {
        if (mesh.vertices.empty()) return {};
        Vec3 mn{static_cast<float>(mesh.vertices[0].x), static_cast<float>(mesh.vertices[0].y), static_cast<float>(mesh.vertices[0].z)};
        Vec3 mx = mn;
        for (const auto& v : mesh.vertices) {
            float vx = static_cast<float>(v.x), vy = static_cast<float>(v.y), vz = static_cast<float>(v.z);
            mn.x = (std::min)(mn.x, vx); mn.y = (std::min)(mn.y, vy); mn.z = (std::min)(mn.z, vz);
            mx.x = (std::max)(mx.x, vx); mx.y = (std::max)(mx.y, vy); mx.z = (std::max)(mx.z, vz);
        }
        return {mn, mx};
    };
    ...
    if (op == Union || (op == SymmetricDiff && disjoint)) {
        GeoMeshData merged;
        merged.vertices = meshA.vertices;
        merged.vertices.insert(merged.vertices.end(), meshB.vertices.begin(), meshB.vertices.end());
        merged.faces = meshA.faces;
        const uint32_t baseIdx = static_cast<uint32_t>(meshA.vertices.size());
        for (const auto& face : meshB.faces)
            merged.faces.push_back({face.v0 + baseIdx, face.v1 + baseIdx, face.v2 + baseIdx});
        merged.normals.resize(merged.vertices.size());
        recalculateNormals(merged);
        result.result = merged;
        result.status = BoolOpResult::Success;
    } else if (op == Difference && disjoint) {
        result.result = meshA;
        result.status = BoolOpResult::Success;
    } else if (op == Intersection && disjoint) {
        result.status = BoolOpResult::Success;
    } else {
        result.status = BoolOpResult::ComputationFailed;
        result.errorMessage = "CGAL not available - overlapping mesh booleans require CGAL";
    }
    result.executionTimeMs = timer.elapsed();
#endif
...
GeoMeshData BooleanOperations::repairMesh(const GeoMeshData& mesh) {
    if (mesh.vertices.empty() || mesh.faces.empty()) return mesh;
    ...
    try {
        ...
        for (auto vertex : cgalMesh.vertices()) {
            ...
            repairedMesh.vertices.push_back(GeoVertex(
                CGAL::to_double(p.x()), CGAL::to_double(p.y()), CGAL::to_double(p.z())));
        }
        for (auto face : cgalMesh.faces()) {
            ...
            for (auto v : cgalMesh.vertices_around_face(h))
                idxs.push_back(static_cast<size_t>(v));
            if (idxs.size() < 3) continue;
            repairedMesh.faces.push_back(GeoFace(
                static_cast<uint32_t>(idxs[0]),
                static_cast<uint32_t>(idxs[1]),
                static_cast<uint32_t>(idxs[2])));
        }
...
void BooleanOperations::recalculateNormals(GeoMeshData& mesh) {
    ...
    for (const auto& face : mesh.faces) {
        if (face.v0 >= mesh.vertices.size() || face.v1 >= mesh.vertices.size() || face.v2 >= mesh.vertices.size())
            continue;
...
BoolOpResult BooleanOperations::performBatchOperations(
    const std::vector<GeoMeshData>& meshes, const std::vector<Operation>& operations) {
