...
void MeshDataBridge::setCurrentMesh(const QVariant& meshData) {
    ...
    m_meshData = graphics::variantToMesh(meshData);
...
void MeshDataBridge::pushToViewport(const QString& meshName) {
    m_currentMeshName = meshName;
    m_currentMesh = graphics::meshToVariant(m_meshData);
...
QVariant MeshDataBridge::loopCut(int cuts, const QVariant& center, const QVariant& normal) {
    QVector3D c = graphics::variantToVector3D(center);
    QVector3D n = graphics::variantToVector3D(normal);

    MeshData result = LoopCut::cut(m_meshData, cuts, c, n);
    m_meshData = result;

    QVariantMap map;
    map["mesh"] = graphics::meshToVariant(result);
...
QVariant MeshDataBridge::knifeCut(const QVariant& start, const QVariant& end, bool snapToVertex) {
    QVector3D s = graphics::variantToVector3D(start);
    QVector3D e = graphics::variantToVector3D(end);
...
QVariant MeshDataBridge::bisectCut(const QVariant& planePoint, const QVariant& planeNormal, bool cutCenter) {
    QVector3D p = graphics::variantToVector3D(planePoint);
    QVector3D n = graphics::variantToVector3D(planeNormal);
...
QVariant MeshDataBridge::subdivide(int levels) {
    ...
    emitMeshModified();
    return graphics::meshToVariant(result);
}
...
void MeshDataBridge::translate(const QVariant& delta) {
    QVector3D d = graphics::variantToVector3D(delta);
...
void MeshDataBridge::rotate(const QVariant& euler) {
    QVector3D e = graphics::variantToVector3D(euler);
...
void MeshDataBridge::scale(const QVariant& factors) {
    QVector3D f = graphics::variantToVector3D(factors);
...
void MeshDataBridge::mirror(const QVariant& axis) {
    QVector3D a = graphics::variantToVector3D(axis);
    a.normalize();
...
QVariant MeshDataBridge::getSelectionBounds() {
    ...
    QVariantMap bounds;
    bounds["min"] = graphics::vector3DToVariant(min);
    bounds["max"] = graphics::vector3DToVariant(max);
    bounds["center"] = graphics::vector3DToVariant((min + max) / 2);
    bounds["size"] = graphics::vector3DToVariant(max - min);
...
QVariant MeshDataBridge::getVertexPositions() {
    ...
    for (const Vertex& v : m_meshData.vertices) {
        positions.append(graphics::vector3DToVariant(v.position));
    }
    return positions;
}
    ...
    for (const Face& f : m_meshData.faces) {
        ...
        for (int idx : f.indices) {
            if (idx < m_meshData.vertices.size()) {
                face.append(graphics::vector3DToVariant(m_meshData.vertices[idx].position));
            }
        }
        faces.append(face);
    }
...
QVariant MeshDataBridge::getNormals() {
    ...
    for (const Vertex& v : m_meshData.vertices) {
        normals.append(graphics::vector3DToVariant(v.normal));
    }
    return normals;
}
...
void MeshDataBridge::updateMeshDataFromVariant() {
    m_meshData = graphics::variantToMesh(m_currentMesh);
}
