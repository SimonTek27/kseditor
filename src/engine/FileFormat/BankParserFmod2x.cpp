...
ParsedBankData BankParserFmod2x::parse(const QString& bankPath) {
    ...
    result.detectedVersion = BankVersion::FMOD_2_02;
    result.detectedGame = m_gameTarget;
...
ParsedBankData BankParserFmod2x::parseFromData(const QByteArray& rawData) {
    ParsedBankData result;
    ...
    if (magic == FMOD_BANK_MAGIC_FEV2) {
        // FMOD 2.x typically doesn't use XOR encryption
        if (rawData.size() >= 12) {
            quint32 flags = 0;
        ...
        result.version = 0;
        qWarning() << "BankParserFmod2x: Legacy RIFF/FEV bank — limited parsing";
    } else {
...
bool BankParserFmod2x::canParse(const QByteArray& data) const {
    if (data.size() < 4) return false;
    quint32 magic = 0;
    memcpy(&magic, data.constData(), 4);
    return magic == FMOD_BANK_MAGIC_FEV2;
}

bool BankParserFmod2x::isValidBank(const QString& bankPath) const {
    QFile file(bankPath);
...
bool BankParserFmod2x::isEncrypted(const QString& bankPath) const {
    QFile file(bankPath);
...
bool BankParserFmod2x::extractAudioData(ParsedBankData& bankData) const {
    return !bankData.sounds.isEmpty();
}

QStringList BankParserFmod2x::getEventPaths(const QString& bankPath) const {
    const_cast<BankParserFmod2x*>(this)->parse(bankPath);
    QStringList paths;
    if (m_cache.contains(bankPath))
...
QStringList BankParserFmod2x::getEventNames(const QString& bankPath) const {
    const_cast<BankParserFmod2x*>(this)->parse(bankPath);
    QStringList names;
    if (m_cache.contains(bankPath))
...
QStringList BankParserFmod2x::getBusPaths(const QString& bankPath) const {
    const_cast<BankParserFmod2x*>(this)->parse(bankPath);
    QStringList paths;
    if (m_cache.contains(bankPath))
...
QStringList BankParserFmod2x::getVCAPaths(const QString& bankPath) const {
    const_cast<BankParserFmod2x*>(this)->parse(bankPath);
    QStringList paths;
    if (m_cache.contains(bankPath))
...
// ============================================================================
// FMOD 2.x FEV2 Parser Implementation
// ============================================================================

bool BankParserFmod2x::parseFEV2(const QByteArray& data, ParsedBankData& out) {
    QDataStream s(data);
    ...
    while (!s.atEnd()) {
        ...
        switch (chunkTag) {
        case CHUNK_STRT:   readChunkStringTable(s, out, chunkSize); break;
        case CHUNK_EVTS:   readChunkEvents(s, out, chunkSize); break;
        case CHUNK_BUSS:   readChunkBuses(s, out, chunkSize); break;
        case CHUNK_VCAS:   readChunkVCAs(s, out, chunkSize); break;
        case CHUNK_SNAP:   readChunkSnapshots(s, out, chunkSize); break;
        case CHUNK_SNDS:   readChunkSounds(s, out, chunkSize); break;
        case CHUNK_PLUG:   readChunkPlugins(s, out, chunkSize); break;
        case CHUNK_MIXR:   readChunkMixer(s, out, chunkSize); break;
        case CHUNK_BANK:   readChunkBankInfo(s, out, chunkSize); break;
        default: break;
        }
...
void BankParserFmod2x::readChunkStringTable(QDataStream& s, ParsedBankData& out, quint32 size) {
    Q_UNUSED(out);
...
void BankParserFmod2x::readChunkEvents(QDataStream& s, ParsedBankData& out, quint32 /*size*/) {
    quint32 count; s >> count;
    for (quint32 i = 0; i < count; ++i) {
        BankEventInfo ev;
...
void BankParserFmod2x::readChunkBuses(QDataStream& s, ParsedBankData& out, quint32 /*size*/) {
    quint32 count; s >> count;
    for (quint32 i = 0; i < count; ++i) {
        BankBusInfo bus;
...
void BankParserFmod2x::readChunkVCAs(QDataStream& s, ParsedBankData& out, quint32 /*size*/) {
    quint32 count; s >> count;
    for (quint32 i = 0; i < count; ++i) {
        BankVCAInfo vca;
        quint32 nameIdx, pathIdx; s >> nameIdx >> pathIdx;
...
void BankParserFmod2x::readChunkSnapshots(QDataStream& s, ParsedBankData& out, quint32 /*size*/) {
    quint32 count; s >> count;
    for (quint32 i = 0; i < count; ++i) {
...
void BankParserFmod2x::readChunkSounds(QDataStream& s, ParsedBankData& out, quint32 /*size*/) {
    quint32 count; s >> count;
    for (quint32 i = 0; i < count; ++i) {
        BankSoundInfo snd;
...
void BankParserFmod2x::readChunkPlugins(QDataStream& s, ParsedBankData& /*out*/, quint32 size) {
    // FMOD 2.x plugin chunk - skip for now
    qint64 start = s.device()->pos();
    s.device()->seek(start + size);
}

void BankParserFmod2x::readChunkMixer(QDataStream& s, ParsedBankData& /*out*/, quint32 size) {
    // FMOD 2.x mixer chunk - skip for now
    qint64 start = s.device()->pos();
    s.device()->seek(start + size);
}

void BankParserFmod2x::readChunkBankInfo(QDataStream& s, ParsedBankData& /*out*/, quint32 size) {
    // FMOD 2.x bank info chunk - skip for now
    qint64 start = s.device()->pos();
    s.device()->seek(start + size);
}

// ============================================================================
// FSB5 Parsing (shared with 1.x but can be extended for 2.x differences)
// ============================================================================

bool BankParserFmod2x::parseFSB5Data(const QByteArray& bankData, ParsedBankData& out) const {
...
QString BankParserFmod2x::stringAt(quint32 idx) const {
    if (idx < (quint32)m_stringTable.size())
        return m_stringTable.at(idx);
    return QString("str_%1").arg(idx);
}

QString BankParserFmod2x::formatGUID(const quint8 bytes[16]) {
...
QByteArray BankParserFmod2x::decryptData(const QByteArray& data, quint32 key) const {
