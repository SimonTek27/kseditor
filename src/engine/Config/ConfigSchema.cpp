...
void ConfigSchemaRegistry::registerBuiltinSchemas() {
    ...
    registerSchema("streamline", streamlineConfigSchema());
}
...
std::shared_ptr<JsonSchema> ConfigSchemaRegistry::streamlineConfigSchema() {
    SchemaBuilder builder;
    builder.object("Streamline Configuration")
        .title("NVIDIA Streamline (DLSS/Reflex/NIS)")
        .description("Configure NVIDIA Streamline SDK features for upscaling, low latency, and image sharpening")
        .property("dlss", SchemaBuilder().object("DLSS - Deep Learning Super Sampling")
            .property("enabled", SchemaBuilder().boolean("Enable DLSS").defaultValue(false).title("Enable DLSS"))
            .property("quality", SchemaBuilder().string("DLSS Quality")
                .enumValues({"max_performance", "balanced", "max_quality", "ultra_performance", "ultra_quality", "dlaa"})
                .defaultValue("balanced")
                .title("Quality Mode"))
            .property("sharpness", SchemaBuilder().number("Sharpness").defaultValue(0.0).minimum(-1.0).maximum(1.0).title("Sharpness"))
            .build())
        .property("reflex", SchemaBuilder().object("Reflex - Low Latency")
            .property("enabled", SchemaBuilder().boolean("Enable Reflex").defaultValue(false).title("Enable Reflex"))
            .property("mode", SchemaBuilder().string("Reflex Mode")
                .enumValues({"enabled", "enabled_plus_boost"})
                .defaultValue("enabled")
                .title("Mode"))
            .build())
        .property("nis", SchemaBuilder().object("NIS - NVIDIA Image Scaling")
            .property("enabled", SchemaBuilder().boolean("Enable NIS").defaultValue(false).title("Enable NIS"))
            .property("quality", SchemaBuilder().string("NIS Quality")
                .enumValues({"on"})
                .defaultValue("on")
                .title("Quality"))
            .property("sharpness", SchemaBuilder().number("Sharpness").defaultValue(0.5).minimum(0.0).maximum(1.0).title("Sharpness"))
            .build());
    
    auto schema = std::make_shared<JsonSchema>();
    schema->id = "streamline";
    schema->title = "Streamline Configuration";
    schema->root = builder.build();
    return schema;
}
