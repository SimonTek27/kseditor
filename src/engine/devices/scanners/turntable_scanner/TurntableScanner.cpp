...
#include "sdk/kseditor/modules/modellingEditor/VulkanViewportItem.h"
#include <QMatrix4x4>
#include <QDateTime>
#include <QFile>
#include <QTextStream>
...
void TurntableScanner::captureFromViewportAtPose(const TurntablePose& pose)
{
    ...
    QTimer::singleShot(100, this, [this, pose]() {
        auto handleCapture = [this, pose](const QImage& srcImg) {
            QImage img = srcImg.isNull() ? QImage(640, 480, QImage::Format_RGB32) : srcImg;
            if (srcImg.isNull()) img.fill(Qt::darkGray);
            QString filename = "scan_image_" + QString::number(m_currentIndex) + ".png";
            bool saved = img.save(filename);
            Q_UNUSED(saved);
            m_images.append(img);
            m_progress.capturedImages = m_currentIndex + 1;
            m_progress.progressPercentage = (static_cast<float>(m_currentIndex + 1) / m_numPositions) * 100.0f;
            QString stepName = "Position " + QString::number(m_currentIndex + 1) + "/" + QString::number(m_numPositions);
            m_progress.currentStep = stepName;
            emit imageCaptured(img, pose);
            emit scanProgressed(m_progress);
            emit stepChanged(m_currentIndex);
            if (m_currentIndex < m_numPositions - 1) {
                QTimer::singleShot(200, this, [this]() { advancePosition(); });
            } else {
                emit scanCompleted("Scan complete - " + QString::number(m_images.size()) + " images captured");
                m_progress.scanning = false;
                emit scanProgressed(m_progress);
            }
        };
        QSharedPointer<QQuickItemGrabResult> grab = m_viewport->grabToImage(QSize(640, 480));
        if (!grab) {
        ...
        connect(grab.data(), &QQuickItemGrabResult::ready, this, [handleCapture, grab]() {
            handleCapture(grab->image());
        });
    });
}
