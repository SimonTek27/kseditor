...
struct TurntablePose {
    ...
    qreal turntableRadius = 0.0;
    qreal cameraHeight = 0.0;
    QDateTime timestamp;
...
public:
    ...
    Q_INVOKABLE void captureCurrentImage();
    
    // Advance to next position
    Q_INVOKABLE void advancePosition();
    
    // Get progress status
    Q_INVOKABLE ScanProgress progress() const { return m_progress; }
    
    // Get all captured poses
    Q_INVOKABLE QList<TurntablePose> capturedPoses() const { return m_poses; }
    
    // Get captured images
    Q_INVOKABLE QList<QImage> capturedImages() const { return m_images; }
    
    // Export in photogrammetry-friendly format
    Q_INVOKABLE QString exportForPhotogrammetry(const QString& directory);
    
    // Set turntable radius
    Q_INVOKABLE void setTurntableRadius(qreal radius) { m_turntableRadius = radius; }
    
    // Set camera height above turntable
    Q_INVOKABLE void setCameraHeight(qreal height) { m_cameraHeight = height; }
...
private:
    ...
    TurntablePose computePoseAtIndex(int index) const;
    void captureFromViewportAtPose(const TurntablePose& pose);
    ...
    ks::VulkanViewportItem* m_viewport = nullptr;
};
