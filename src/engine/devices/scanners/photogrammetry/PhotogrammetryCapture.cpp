...
namespace ks {
namespace device {
// capturedImages() is defined inline in the header
...
QString PhotogrammetryCapture::exportColmapFormat(const QString& directory) const
{
    ...
    if (f_images.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        for (int i = 0; i < m_capturedImages.size(); ++i) {
            ...
            out << "image_" << i << ".png, "
                << pose.rotation.scalar() << " "
                << pose.rotation.vector().x() << " "
                << pose.rotation.vector().y() << " "
                << pose.rotation.vector().z() << " , "
    ...
    const_cast<PhotogrammetryCapture*>(this)->emit exportFinished(result);
    return result;
}
...
void PhotogrammetryCapture::captureFromViewport()
{
    ...
    QTimer::singleShot(50, this, [this]() {
        QSharedPointer<QQuickItemGrabResult> grab = m_viewport->grabToImage(QSize(int(m_imageWidth), int(m_imageHeight)));
        if (!grab) {
            QImage fallback(int(m_imageWidth), int(m_imageHeight), QImage::Format_RGB32);
            fallback.fill(Qt::darkGray);
            CameraPose pose = computeCameraPose();
            CapturedImage ci; ci.image = fallback; ci.pose = pose; ci.valid = true;
            m_capturedImages.append(ci);
            emit capturesChanged();
            emit imageCaptured(fallback, pose);
            return;
        }
        connect(grab.data(), &QQuickItemGrabResult::ready, this, [this, grab]() {
            QImage img = grab->image();
            if (img.isNull()) {
                img = QImage(int(m_imageWidth), int(m_imageHeight), QImage::Format_RGB32);
                img.fill(Qt::darkGray);
            }
            CameraPose pose = computeCameraPose();
            CapturedImage ci; ci.image = img; ci.pose = pose; ci.valid = true;
            m_capturedImages.append(ci);
            emit capturesChanged();
